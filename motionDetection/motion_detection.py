# To make this script in live enter in directory before
import cv2
import socket
# import posix_ipc
...
positionCalculator = PositionCalculator('C:\\Users\Airat\Desktop\FINAL\libcalc_pos.so')
...
#             break
#         frame = read_image()
path_input_video = '/home/kali/Desktop/in.avi'
path_out_video = '/home/kali/Desktop/out.avi'

cap = cv2.VideoCapture(path_input_video)  # Read input videofile
out = cv2.VideoWriter(path_out_video, cv2.VideoWriter_fourcc(*'XVID'), 20.0, (640, 512))  # Write output videofile

while cap.isOpened():
    ret, frame = cap.read()
    # cv2.imshow('',frame) # We can see just input video
    if ret:
        ...
        # cv2.imshow('',img) # We can see just input video in HSV color palet
        __, __, v = cv2.split(img)
        img = v
        # cv2.imshow('',img) # Grayscale input video

        foreground = bgSub.apply(img)
        cv2.imshow('', foreground)  # Detecting motion video without any filtres
        if numFrame > 0:
            __, bwFG = cv2.threshold(src=foreground, thresh=127, maxval=1, type=cv2.THRESH_BINARY)
            # cv2.imshow('',bwFG) # See background

            blockMask = buildBlockMask(bwFG, r=r, rho=rho)
            # cv2.imshow('', blockMask) # Here detecting motion video with filtre
            blockMask = segmentationBlockMask(blockMask)
            # cv2.imshow('', blockMask)
